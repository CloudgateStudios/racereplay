...
the event results page. Already specced in NEW_FUNCTIONALITY.md.
...
Password-protected browser-based CSV upload. Already specced.
...
### N7 — Email / push notifications
"Alert me when results for [race] are posted." Requires an email or push
infrastructure (Resend, etc.) and some kind of waitlist/subscription model.

**Schema dependency:** New `Subscription` or `WaitlistEntry` model needed.
Out of scope until the race catalog grows significantly.

---

### N8 — Embeddable widgets
